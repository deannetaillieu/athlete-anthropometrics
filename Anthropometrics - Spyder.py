...
inverted = ~duplicates # Invert boolean array so athletes with 1 measurement are shown as 'True'
non_duplicates_index = [i for i, val in enumerate(inverted) if val] # Find the index of athletes with only 1 measurement
duplicates_index = [i for i, val in enumerate(duplicates) if val] # Find the index of athletes with 2+ measurements
...
writer = pd.ExcelWriter("anthropometric_dataframe.xlsx", # Create a Pandas Excel writer using XlsxWriter as the engine  
                        engine='xlsxwriter',
                        datetime_format='mmm d yyyy') # Set the default datetime format 
anthro_data.to_excel(writer, sheet_name='Sheet1', index=False) # Convert the DataFrame to an XlsxWriter Excel object
